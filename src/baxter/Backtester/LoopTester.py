...
class LoopTester(Backtester):
    ...
    def _calculate_equity_curve(self) -> None:
        ...
        self._equity_curve["Realized Dollar P&L"] = pd.Series(
            self._loop_results['pnls'], index=self._results['trade_dates']).cumsum()

        self._equity_curve["Unrealized Dollar P&L"] = pd.Series(
            self._loop_results['unrlzd'], index=self._loop_results['dates']).cumsum()

        self._equity_curve["Realized Benchmark P&L"] = self.benchmark.diff(
        ).cumsum()

        self._equity_curve["Realized Pct. P&L"] = (
            1 + pd.Series(self._loop_results['pnls_pct'], index=self._loop_results['trade_dates'])).cumprod()

        self._equity_curve["Unrealized Pct. P&L"] = (
            1 + pd.Series(self._loop_results['unrlzd_pct'], index=self._loop_results['dates'])).cumprod()

        self._equity_curve["Unrealized Benchmark P&L"] = (
            1 + self.benchmark.pct_change()).cumprod()

        self._equity_curve["Unrealized Drawdown"] = self._equity_curve["Unrealized Pct. P&L"] / \
            self._equity_curve["Unrealized Pct. P&L"].expanding().max() - 1
        self._equity_curve["Realized Drawdown"] = self._equity_curve["Realized Pct. P&L"] / \
            self._equity_curve["Realized Pct. P&L"].expanding().max() - 1
